...
- **Execution plan**: this commit is doctrine only. Tier 1 execution (load Fraunces, rebuild prayer-home hero, rebuild landing hero, voice sweep) lands in separate surface-specific commits per the new rules. Line art assets are generated by the user via Runware / Nano Banana 2 using prompts in `assets/line-art/PROMPTS.md` (to be created at first-asset time).
